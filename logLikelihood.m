function l = logLikelihood(y, pi_1, P, emission_means, emission_covs,...
    zero_probs)
% Returns the likelihood of the parameters given one person's data
...
%                   current parameters given the data
...
%     All  arguments must be input
    if nargin < 6
        error('all 6 arguments must be input')
    end
...
% %     l = sum(log(l));
    ...
    y_conditional_likelihoods = zeros(s_y(1), 1);
    zero_probs = [ones(s_y(1), 1), zero_probs];
    x_conditional_likelihoods = cell2mat(arrayfun(@(x_idx)...
        mvnpdf(y(:, :, x_idx), emission_means(:, :, x_idx),...
        emission_covs(:, :, x_idx)) .* zero_probs(:, x_idx), 1:s_y(3),...
        'UniformOutput', false));
    alpha_t = pi_1 .* x_conditional_likelihoods(1, :);
    y_conditional_likelihoods(1) = sum(alpha_t);
    filtered_x = alpha_t ./ y_conditional_likelihoods(1);
    for t = 2:s_y(1)
