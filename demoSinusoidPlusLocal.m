...
x = -20:0.2:20; 
y = displac + scale^2*cos(2*pi*x/period) + normrnd(0,noise,1,length(x));
...
covfunc = {@covSum, {@covCos, @covSEiso}}; hyp.cov = log([period, ((max(y)-min(y))/2)^2, 1, 1]);
...
hyp.cov = log([period + normrnd(0,1), ((max(y)-min(y))/2)^2 + normrnd(0,1), 1, 1]);
hyp.lik = log(sn + 1);
