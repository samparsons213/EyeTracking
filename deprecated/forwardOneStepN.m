function p_x = forwardOneStepN(y, p_x_prev, P, p_ugx, emission_means,...
    emission_covs, root_cov_dets, zero_probs, n_timesteps)
% Performs both the forward predict and forward update of one step of the
% forward pass, returning p(x_t | y_1:t, u_1:t), allowing for non-adjacent
% data, i.e. the next data point can be more than one time step away.
...
% n_timesteps:      positive integer giving the number of timesteps to
%                   propagate the transition dynamics through
...
%     First 3 arguments must be input
    if nargin < 8
        error('the first 8 arguments must be input')
    end
