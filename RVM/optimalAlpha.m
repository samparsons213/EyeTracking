function [alpha, w, relevant_dims] = optimalAlpha(x, y, epsilon, max_iters, eff_inf, alpha)
...
% alpha:        D by C array of positive reals giving initial estimate of
%               alpha (optional)

% Outputs:

% alpha:        D by C array of positive reals giving final estimate of
%               alpha
...
%     If input, alpha must be a [D, C] non-negative array
    if nargin > 5
        if ~(isnumeric(alpha) && isreal(alpha) && ismatrix(alpha) &&...
                all(size(alpha) == [D, C]) && all(alpha(:) >= 0))
...
%         alpha_old = alpha;
        ...
        w = minFunc(@(theta) weightPosteriorSBLR(theta, x, y, alpha, true),...
            w(:), opts);
        fprintf('\tOptimising alpha\n')
        [~, ~, H] = weightPosteriorSBLR(w, x, y, alpha, false);
        Sigma = -inv(H);
        alpha = reshape(1 ./ (w .^ 2 + diag(Sigma)), D, C);
% %         alpha = reshape((1 - diag(Sigma).*alpha(:)) ./ (w.^2), D, C);
