...
%     each element of y must be a [n_p 2 dim_x] real array, and each element
%     of p_x must be a [n_p dim_x] array of probability distruibutions
%     (each row sums to 1), and each element of p_x_x_next must be a
%     [dim_x dim_x n_p-1] array of probability matrices (each matrix sums
%     to 1)
    n_people = length(y);
    dim_x = size(y{1}, 3);
...
%         Check on y
        if ~(isnumeric(y{person_idx}) && isreal(y{person_idx}) &&...
                (ndims(y{person_idx}) == 3) &&...
                all(s_y(2:3) == [2, dim_x]))
            error('each y must be a [n_p 2 dim_x] real array')
        end
%       Check on p_x
        if ~(isnumeric(p_x{person_idx}) && isreal(p_x{person_idx}) &&...
                all(size(p_x{person_idx}) == [s_y(1), dim_x]))
            error('each p_x must be a [size(y, 1), size(y, 3)] real array')
        end
        num_tol = 1e-8;
        is_prob_dist = arrayfun(@(row_idx)...
            all(p_x{person_idx}(row_idx, :) >= 0) &&...
            (abs(1 - sum(p_x{person_idx}(row_idx, :))) < num_tol),...
            1:s_y(1));
        if ~all(is_prob_dist)
            error('all rows of each p_x must be probability distributions')
        end
%       Check on p_x_x_next
        if ~(isnumeric(p_x_x_next{person_idx}) && isreal(p_x_x_next{person_idx}) &&...
                all(size(p_x_x_next{person_idx}) == [dim_x, dim_x, s_y(1)-1]))
            err_msg = ['each p_x_x_next must be a [dim_x, dim_x,',...
        ...
        is_prob_dist = arrayfun(@(row_idx)...
            ...
            1:(s_y(1)-1));
        if ~all(is_prob_dist)
            err_msg = ['all matrices of each p_x_x_next must be',...
...
%     *********************************************************************
    ...
    for person_idx = 1:n_people
        ...
        emission_covs(:, :, :, person_idx) =...
            squeeze(sum(bsxfun(@times, diffs_sq, weights_person), 3));
    end
